...
export async function getGazaDestructionData() {
  try {
      ...
      const formattedDate = latestReportDateStr.split('-').reverse().join('.');

      console.log("Formatted Gaza Destruction Date:", formattedDate);

      return { latestReport, formattedGazaDestructionDate: formattedDate, fullData: data };
  } catch (error) {
