...
async function fetchNonprofitData(ein) {
  try {
    ...
    // Extract the name and total revenue over the last 10 years
    const organizationName = jsonResponse.organization.name;
    const recentFilings = jsonResponse.filings_with_data.slice(0, 10); // Get the last 10 years
    const totalRevenue = recentFilings.reduce(
      (sum, filing) => sum + filing.totrevenue,
      0
    );

    return { name: organizationName, value: totalRevenue };
  } catch (error) {
 ...
 * @param {string[]} eins - Array of EINs
...
/**
 * Get treemap data for nonprofits.
 * @returns {Promise<{name: string, value: number, children: {name: string, value: number}[]}>}
 */
export async function getNGOTreemapData() {
  const rawData = await fetchDataForEINs(eins);
  const totalCombinedRevenue = rawData.reduce((sum, org) => sum + org.value, 0);
  return {
    name: "Total",
    value: totalCombinedRevenue, // Total revenue of all organizations combined
    children: rawData,
  };
}
